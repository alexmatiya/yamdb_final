...
class Title(models.Model):
    ...
    year = models.IntegerField(verbose_name="Год", validators=[check_year])
...
class GenreTitle(models.Model):
    ...
    genre_id = models.ForeignKey(
        Genre, on_delete=models.CASCADE, verbose_name="ID жанра"
    )
...
class Review(ReviewAndCommentModel):
    ...
    class Meta(ReviewAndCommentModel.Meta):
        constraints = [
            UniqueConstraint(name="unique_author_title",
                             fields=["author", "title"])
        ]
        verbose_name = "Отзывы"
